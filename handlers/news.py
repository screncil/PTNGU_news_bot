...
async def dislineMedia(msg: Message, state: FSMContext):
    ...
    await msg.answer("Повідомлення відправлено до адміністратора 😙", reply_markup=ReplyKeyboardRemove())
...
async def acceptt(call: CallbackQuery, state: FSMContext):
    ...
    if "media" in data:
        if isinstance(data["media"], list):
            ...
            await bot.bot.send_message(chat_id=config.CHANNEL_ID,
            ...
            await bot.bot.send_photo(chat_id=config.CHANNEL_ID, photo=data['media'])
            await bot.bot.send_message(chat_id=config.CHANNEL_ID,
                                       text=f"<b>{data['title']}</b>\n\n<i>{data['description']}</i>\n\n{datetime.now().strftime('%Y/%m/%d %H:%M')}",
                                       parse_mode=ParseMode.HTML)
            await state_with.clear()
...
async def newsdisline(call: CallbackQuery, state: FSMContext):
    ...
    await state_with.clear()
    await call.message.delete()
    await call.message.answer("Новина відхилена ❌")
    await bot.bot.send_message(chat_id=user[-1], text="Ваша новина відхилена адміністратором ❌")
...
async def mediaa(msgs: List[Message], state: FSMContext):
    ...
    for admin in admin_db.getAllAdminIds():
        ...
        await bot.bot.send_message(chat_id=admin,
                                   text=f"<b>Нове повідомлення від <a href='tg://user?id={data['user_id']}'>{data['username']}</a></b>\n\n{'=' * 20}\n\n<b>{data['title']}</b>\n\n<i>{data['description']}</i>",
...
async def photo(msg: Message, state: FSMContext):
    ...
    data = await state_with.update_data(data=data_)
    for admin in admin_db.getAllAdminIds():
        await bot.bot.send_photo(chat_id=admin, photo=data['media'])
        await bot.bot.send_message(chat_id=admin,
                                   text=f"<b>Нове повідомлення від <a href='tg://user?id={data['user_id']}'>{data['username']}</a></b>\n\n{'=' * 20}\n\n<b>{data['title']}</b>\n\n<i>{data['description']}</i>",
                                   parse_mode=ParseMode.HTML, reply_markup=NewsKB(data['user_id']).as_markup())
